...
class RedmineAiHelper::LlmTest < ActiveSupport::TestCase
  ...
  def test_select_tool
    ...
    assert_equal "project_tool_provider", result["tool"]["provider_name"]
  ...
  class DummyOpenAIClient
    def chat(params = {})
      ...
      if message.include?("タスクに対する最終回答を作成してください")
        ...
        answer = { "tool" => { "provider_name" => "project_tool_provider", "tool" => "read_project", "arguments" => { "id": ["1"] } } }.to_json
        if message.include?("dispatch_success_test")
          answer = { "tool" => { "provider_name" => "project_tool_provider", "tool" => "read_project", "arguments" => { "id": ["1"] } } }.to_json
        elsif message.include?("execute_task_error")
          answer = { "tool" => { "provider_name" => "project_tool_provider", "tool" => "read_project", "arguments" => { "id": ["999"] } } }.to_json
        elsif message.include?("dispatch_error")
          answer = { "tool" => { "provider_name" => "aaaa", "tool" => "read_project", "arguments" => { "id": ["999"] } } }.to_json
        end
