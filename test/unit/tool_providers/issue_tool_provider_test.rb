  ...
  context "IssueToolProvider" do
    ...
    context "create_new_issue" do
      ...
      context "validate_only is true" do
        ...
        should "return error with invalid project" do
          response = @provider.create_new_issue(project_id: 1, tracker_id: 999, status_id: 1, subject: "test issue", description: "test description", validate_only: true)
          assert response.is_error?
    ...
    context "update_issue" do
      ...
      context "validate_only is true" do
        should "validate issue" do
          ...
          response = @provider.update_issue(issue_id: 1, subject: "test issue", validate_only: true)
          assert response.is_success?
          assert_equal original_subject, Issue.find(1).subject
        end

        should "return error with invalid id" do
          response = @provider.update_issue(issue_id: 1, tracker_id: 999, subject: "test issue", validate_only: true)
          assert response.is_error?
          assert response.error.include?("Validation failed")
          assert response.error.include?("Tracker")
        end
