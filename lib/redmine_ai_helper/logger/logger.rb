module RedmineAiHelper
  module Logger
    ...
    module ClassMethods
      ...
      def debug(message)
        mylogger.debug("[#{self.name}] #{message}")
      end

      def info(message)
        mylogger.info("[#{self.name}] #{message}")
      end

      def warn(message)
        mylogger.warn("[#{self.name}] #{message}")
      end

      def error(message)
        mylogger.error("[#{self.name}] #{message}")
      end
    end
    ...
    def debug(message)
      mylogger.debug("[#{self.class.name}] #{message}")
    end

    def info(message)
      mylogger.info("[#{self.class.name}] #{message}")
    end

    def warn(message)
      mylogger.warn("[#{self.class.name}] #{message}")
    end

    def error(message)
      mylogger.error("[#{self.class.name}] #{message}")
    end
  end
